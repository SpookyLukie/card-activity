...
public class StandardFaceValue implements FaceValue, Comparable<StandardFaceValue> {
    ...
    public StandardFaceValue(String name, int ordinal) {
        this.name = name;               //variable shadowing (the first <name> in purple references String name
        this.ordinal = ordinal;         //variable shadowing (this.<ordinal> references private int ordinal
    }

    public static Set<FaceValue> value() {
    ...
    public int compareTo(StandardFaceValue that) {
        ...
        if (this.ordinal > that.ordinal) {      //sorting algorithm
            return 1;
